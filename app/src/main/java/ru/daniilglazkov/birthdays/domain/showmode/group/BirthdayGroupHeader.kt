...
interface BirthdayGroupHeader {
    ...
    class Age(now: LocalDate) : Abstract(
        MakeHeader.Range(
            RangeTextFormat.Base(),
            BirthdayGroupHeaderPredicate.Range(DateDifference.TurnsYearsOld(now))
        )
    )

    class MonthAndYear(nextEvent: NextEvent) : Abstract(
        MakeHeader.BasedOnFirst(BirthdayGroupHeaderPredicate.MonthAndYear(nextEvent))
    )

    class Date(format: DateTextFormat) : Abstract(
        MakeHeader.BasedOnFirst(BirthdayGroupHeaderPredicate.Date(format))
    )

    class FirstCharOfName : Abstract(
        MakeHeader.BasedOnFirst(BirthdayGroupHeaderPredicate.FirstChar())
    )
    ...
    class Zodiac(
        ...
        MakeHeader.BasedOnFirst(
            BirthdayGroupHeaderPredicate.Zodiac(classification, zodiacDomainToHeaderMapper)
        )
    )
}